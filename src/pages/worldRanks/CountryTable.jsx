...
function CountryTable() {
  ...
  function createUrlLink(item) {
    ...
    const currencyArray = [];
    if (currencies !== "none") {
      const currencyObject = Object.values(currencies);

      for (let i = 0; i < currencyObject.length; i++) {
        currencyArray.push(currencyObject[i].name);
      }
    }

    const languagesObject = Object.values(languages);

    const currencyString =
      currencies === "none" ? currencies : currencyArray.join("-");
    const languageString =
      languages === "none" ? languages : languagesObject.join("-");

    const urlString = `/country/${common}/${official}/${flag}/${population}/${area}/${capital.toString()}/${subregion}/${languageString}/${currencyString}/${region}`;
                  ...
                  paginatedTableData.map((item) => {
                    return (
                      <tr key={item.name.common} className="wr__table-data">
                        ...
                        <td>
                          <Link
                            to={createUrlLink(item)}
                            className="wr__table-name-link"
                          >
                            {item.name.common.length > 30
                              ? `${item.name.common.slice(0, 30)}...`
                              : item.name.common}
                          </Link>
                        </td>
